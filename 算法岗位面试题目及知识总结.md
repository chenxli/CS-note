...
34. batch normalization和layer normalization区别

    公式一样，不过bn的均值和标准差是用一个batch计算出来的，ln样本具有独立性，是该样本所有特征计算而来的。
...
1. self-attention如何实现的，介绍了一下原理
   ...
   具体公式：$SA(K,Q,V)=softmax \frac{Q \cdot K^T}{\sqrt{d}}V$
...
33. FM是怎么降低复杂度的？
    ...
    使用了如$(a+b+c)^2-a^2-b^2-c^2$的技巧

    ![img](https://img-blog.csdnimg.cn/img_convert/7d46e4d09cff4ea84ac289084695ee1f.png)

34. 多路召回有哪些思路？为什么要进行多路召回？

    ![preview](https://pic4.zhimg.com/v2-6291a51c3d710a1486a78e546a322627_r.jpg)

    每一路召回相当于是在做单特征排序，比如热门召回是根据物品受欢迎程度进行排序，兴趣标签是根据贴近用户兴趣标签的程度进行排序。
