...
1. xgb、lgb、catboost区别
   ...
   |      | xgb                                                          | lgb                                                   | cat  |
   | ---- | ------------------------------------------------------------ | ----------------------------------------------------- | ---- |
   | 优点 | 二阶泰勒展开，残差近似更加准确；可以对缺失值进行处理；易并行； | 直方图算法，生内存，同时特征分裂的时候只需要计算K次。 |      |
   | 缺点 | 每个特征都要预排序，空间消耗大；结点分割的时候每次都要整个数据集计算增益，时间消耗也不少 |                                                       |      |
...
18. 参数搜索方法：
    ...
    - 随即搜索：采样，比网格搜索快 RandomizedSearchCV(mode, params,scoring,n_jobs, cv)
